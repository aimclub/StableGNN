...
class Explain:
    ...
    def DataGeneration(self, node_idx, num_samples=100, pred_threshold=0.1):
        ...
        for iteration in range(num_samples):
            ...
            A_torch = torch.tensor([self.A], dtype=torch.float).squeeze()
            data_perturb = Data(
                x=X_perturb_torch, edge_index=A_torch.nonzero().t().contiguous()
            )
            ...
            for node in neighbors:
                if (
                    soft_pred_perturb[node, np.argmax(soft_pred[node])] + pred_threshold
                ) < np.max(soft_pred[node]):
    ...
    def VariableSelection(
        ...
        for node in neighbors:
            chi2, p, _ = chi_square(
                ind_ori_to_sub[node], ind_ori_to_sub[node_idx], [], data, boolean=False
            )
            p_values.append(p)
            if p < p_threshold:
    ...
    def search_MK(self, data, target, nodes):
        ...
        while True:
            ...
            for node in nodes:
                ...
                _, p, _ = chi_square(
                    target, node, evidences, data[nodes + [target]], boolean=False
                )
                if p > 0.05:
                    MB.remove(node)
                    count = 0
