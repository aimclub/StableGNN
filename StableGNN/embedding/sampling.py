...
from .negativeSampling import NegativeSampler
...
class Sampler:
    def __init__(self, datasetname, data, device, loss_info, **kwargs):
        self.device = device
...
class SamplerWithNegSamples(Sampler):
    def __init__(self, datasetname, data, device, loss_info, **kwargs):
        Sampler.__init__(self, datasetname, data, device, loss_info, **kwargs)
    ...
    def neg_sample(self, batch):
        ...
        neg_batch = self.NS.negative_sampling(
            batch, num_negative_samples=self.num_negative_samples
        )
...
class SamplerRandomWalk(SamplerWithNegSamples):
    def __init__(self, datasetname, data, device, loss_info, **kwargs):
        SamplerWithNegSamples.__init__(
            self, datasetname, data, device, loss_info, **kwargs
        )
        self.loss = loss_info
        self.p = self.loss["p"]
        self.q = self.loss["q"]
        ...
        self.context_size = (
            self.loss["context_size"]
            if self.walk_length >= self.loss["context_size"]
            else self.walk_length
        )
...
class SamplerContextMatrix(SamplerWithNegSamples):
    def __init__(self, datasetname, data, device, loss_info, help_dir, **kwargs):
        SamplerWithNegSamples.__init__(
            self, datasetname, data, device, loss_info, **kwargs
        )
        self.loss = loss_info
    ...
    def pos_sample(self, batch, **kwargs):
        ...
        pos_batch = []
        d = datetime.now()
        if self.loss["C"] == "Adj" and (
            self.loss["Name"] == "LINE" or self.loss["Name"] == "Force2Vec"
        ):
            ...
            name = (
                f"{self.help_dir}/pos_samples_VERSEAdj_" + self.datasetname + ".pickle"
            )
            if os.path.exists(name):
                with open(name, "rb") as f:
                    pos_batch = pickle.load(f)
            ...
            if os.path.exists(SimRankName):
                ...
                ASparse = SparseTensor(
                    row=row, col=col, sparse_sizes=(len(batch), len(batch))
                )
                r = 200
                length = list(
                    map(lambda x: x * int(r / 100), [22, 17, 14, 10, 8, 6, 5, 4, 3, 11])
                )  # O(t)
                mask = []
                for i, l in enumerate(
                    length
                ):  # max(l) = (1-sqrt(c)); O((1-sqrt(c))*t^2)
                    mask1 = torch.zeros([l, 10])  # O( (1-sqrt(c)) *t)
                    mask1.t()[: (i + 1)] = 1  # O( 3*(1-sqrt(c)) *t)=O((1-sqrt(c)) *t)
                    mask.append(mask1)
                mask = torch.cat(mask)  # O((1-sqrt(c)) *t^2)
                mask_new = 1 - mask
                A = self.find_sim_rank_for_batch_torch(
                    batch, ASparse, self.device, mask, mask_new, r
                )
                with open(SimRankName, "wb") as f:
                    pickle.dump(A, f)
            samples_name = (
                f"{self.help_dir}/samples_simrank_" + self.datasetname + ".pickle"
            )
            ...
            alpha = self.alpha
            name_of_file = (
                f"{self.help_dir}/pos_samples_VERSEPPR_"
                + str(alpha)
                + "_"
                + self.datasetname
                + ".pickle"
            ...
            if os.path.exists(name_of_file):
                ...
                print("2")
                A = (1 - alpha) * torch.inverse(
                    torch.diag(torch.ones(len(Adg))) - alpha * torch.matmul(invD, Adg)
                )
                print("3")
                pos_batch = self.convert_to_samples(batch, A)
                print("4")
        ...
        if self.loss["loss var"] == "Factorization":
            if self.loss["C"] == "Adj":
                ...
                if True:
                    ...
                    C = (1 - alpha) * torch.inverse(
                        torch.diag(torch.ones(len(A))) - alpha * torch.matmul(invD, A)
                    )
...
class SamplerAPP(SamplerWithNegSamples):
    ...
    def pos_sample(self, batch, **kwargs):
        ...
        if os.path.exists(name_pos_samples):
            ...
            ASparse = SparseTensor(
                row=row, col=col, sparse_sizes=(len_batch, len_batch)
            )
            pos_dict = self.find_PPR_approx(
                batch, ASparse, self.device, self.r, self.alpha, row
            )
            pos_batch = []
    ...
    def find_PPR_approx(self, batch, Adj, device, r, alpha, row):
        ...
        length = list(
            map(lambda x: int((1 - alpha) ** x * alpha * r), list(range(N - 1)))
        )
        r = sum(length)
        dict_data = dict()
        # device = 'cpu'
                ...
                for i, seg in enumerate(split):
                    ...
                    for pos_sample in pos_samples:
                        if (int(seg[0][0]), int(pos_sample)) in dict_data:
                            dict_data[(int(seg[0][0]), int(pos_sample))] += pos_samples[
                                pos_sample
                            ]
                        elif (int(pos_sample), int(seg[0][0])) in dict_data:
                            dict_data[(int(pos_sample), int(seg[0][0]))] += pos_samples[
                                pos_sample
                            ]
                        else:
