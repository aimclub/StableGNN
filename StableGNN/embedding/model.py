...
class Net(torch.nn.Module):
    def __init__(
        ...
        if self.conv == "GCN":
            ...
            if self.num_layers == 1:
                self.convs.append(
                    GATConv(self.num_features, out_channels, heads=self.heads)
                )
            else:
                self.convs.append(
                    GATConv(self.num_features, self.hidden_layer, heads=self.heads)
                )
                for i in range(1, self.num_layers - 1):
                    self.convs.append(
                        GATConv(
                ...
                self.convs.append(
                    GATConv(
                        self.heads * self.hidden_layer, out_channels, heads=self.heads
                    )
                )
    ...
    def lossRandomWalks(self, out, PosNegSamples):
        ...
        pos_rw, neg_rw = pos_rw.type(torch.LongTensor).to(self.device), neg_rw.type(
            torch.LongTensor
        ).to(self.device)
        # Positive loss.
        pos_loss = 0
        ...
        pos_loss = -(
            torch.nn.LogSigmoid()(dot)
        ).mean()  # -torch.log(torch.sigmoid(dot)).mean()
    ...
    def lossContextMatrix(self, out, PosNegSamples):
        ...
        if self.loss_function["Name"] == "LINE":
            ...
            self.loss_function["Name"].split("_")[0] == "VERSE"
            or self.loss_function["Name"] == "APP"
        ):
    ...
    def lossFactorization(self, out, S, **kwargs):
        ...
        loss = 0.5 * sum(
            sum((S - torch.matmul(out, out.t())) * (S - torch.matmul(out, out.t())))
