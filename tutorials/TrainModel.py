...
class TrainModel:
    def __init__(self, data, conv="GAT", device="cuda", task="NC", SSL=False, EXTRAPOLATE=True):
    ...
    def test_GC(self, model, loader, **args):  # ,n_estimators,learning_rate_catboost, max_depth):
        ...
        for dat in loader:
            ...
            y_true = dat.y
            accs_micro.append(f1_score(y_true.cpu().tolist(), y_pred.squeeze().tolist(), average="micro"))
        ...
        if self.task == "GC":
            if self.EXTRAPOLATE:
                ...
                self.train_dataset, self.test_dataset, self.val_dataset = model.Extrapolate(
                    self.train_indices, self.val_indices, init_edges, remove_init_edges, white_list, score_func
                )
            ...
            # self.data = self.train_dataset+self.test_dataset
                ...
                self.train_dataset, self.test_dataset, _, _ = model.convert_dataset(
                    data=self.data, train_indices=self.train_indices, val_indices=self.val_indices
                )
            ...
            train_loader = NeighborSampler(
                self.data.edge_index, node_idx=self.train_mask, batch_size=int(len(self.train_mask)), sizes=[-1] * size,
            )
            test_loader = None
        ...
        if self.task == "GC":
            if self.EXTRAPOLATE:
                ...
                self.train_dataset, _, self.val_dataset, _ = model.convert_dataset(
                    self.data, train_indices=self.train_indices, val_indices=self.val_indices
                )
            # train_loader = NeighborSampler(
            #    self.data.edge_index,
            #   node_idx=self.train_mask,
            ...
            train_loader = NeighborSampler(
                self.data.edge_index, batch_size=int(sum(self.train_mask)), node_idx=self.train_mask, sizes=[-1] * size,
            )
            val_loader = None
