...
class TrainModel:
    ...
    def train(self, model, optimizer, train_loader=None):
        ...
        if self.task == "NC":
            ...
            for dat in train_loader:
                ...
                out, deg_pred = model.forward(
                    batch_x, batch_edge_list, edge_weight, batch
                )
    ...
    @torch.no_grad()
    def test_GC(
        self, model, loader, **args
    ):  # ,n_estimators,learning_rate_catboost, max_depth):
        model.eval()
        accs_micro = []
        accs_macro = []
        for dat in loader:
            ...
            y_true = dat.y
            accs_micro.append(
                accuracy_score(y_true.cpu().tolist(), y_pred.squeeze().tolist())
            )
            accs_macro.append(
                f1_score(
...
class TrainModelOptuna(TrainModel):
    def objective(self, trial):
        ...
        if self.task == "GC":
            ...
            val_acc_mi, val_acc_ma = self.test(
                model, val_loader=val_loader, mask=self.val_mask
            )
