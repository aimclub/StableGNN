...
def fine_tune_client(data_processor_mock, tmp_path):
    ...
    with patch('graph_generator.core.fine_tune_client.AutoModelForCausalLM.from_pretrained') as mock_from_pretrained_model, \
         patch('graph_generator.core.fine_tune_client.AutoTokenizer.from_pretrained') as mock_from_pretrained_tokenizer:
...
def test_prepare_training_data_content(fine_tune_client, data_processor_mock):
    ...
    assert json.loads(entry["completion"]) == {
        "entities": ["привет", "мир", "тестовый", "текст"]
    }
...
def test_download_fine_tuned_model_failure(fine_tune_client, data_processor_mock, tmp_path):
    ...
    with patch('graph_generator.core.fine_tune_client.shutil.copytree', side_effect=Exception("Copy failed")) as mock_copytree:
        # Создание фиктивной тонко настроенной модели
        model_dir = os.path.join(fine_tune_client.output_dir, "fine_tuned_model")
        os.makedirs(model_dir, exist_ok=True)
        with open(os.path.join(model_dir, "pytorch_model.bin"), "wb") as f:
